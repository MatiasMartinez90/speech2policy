...
const CategoryCoursesPage = ({ user, signOut }: { user: any; signOut: any }) => {
  ...
  // Obtener config de la categoría
  const { getCategoryConfig } = useCategories()
  const categoryConfig = getCategoryConfig(categoryKey)

  // Usar el hook useCourses con filtro por categoría
  const { courses, loading, error, fetchCourses } = useCourses()

  useEffect(() => {
    if (categoryKey) {
      // Fetch courses con filtro de categoría actual
      fetchCourses({ category: categoryKey, is_published: true })
    }
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <button
          ...
          className="flex items-center space-x-2 text-gray-400 hover:text-gray-300 mb-8 transition-colors"
        >
        ...
        {/* Hero Destacado - Curso Principal */}
        {!loading && !error && courses.length > 0 && (() => {
          // Obtener curso featured o el más reciente
          const featuredCourse = courses.find(c => c.featured) || courses[0]

          return featuredCourse && (
            <div className="mb-12">
              <div className="bg-gradient-to-br from-slate-800/90 via-slate-800/95 to-slate-900/90 backdrop-blur-sm border border-slate-700/50 rounded-3xl p-10 relative overflow-hidden">
                {/* Background glow */}
                <div className={`absolute inset-0 bg-gradient-to-r ${categoryConfig.color} opacity-10 rounded-3xl blur-3xl`}></div>

                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      {/* Badge */}
                      <span className="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-purple-500 to-blue-500 text-white shadow-lg mb-4">
                        {categoryConfig.emoji} CURSO ESTRELLA
                      </span>

                      {/* Título y descripción */}
                      <h2 className="text-3xl font-bold text-white mb-2">{featuredCourse.course_name}</h2>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-gray-400 text-sm ml-2">
                        {featuredCourse.student_count || 0}+ estudiantes
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-6 mb-6">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-white">{featuredCourse.estimated_time || 'N/A'}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-white">{featuredCourse.difficulty}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-white">${(featuredCourse.cost || 0).toFixed(2)}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-center">
                    <button
                      onClick={() => goToCourse(featuredCourse.course_id)}
                    ...
                    >
                      ...
                      <span className="relative z-10 flex items-center justify-center space-x-2">
                        <span>🚀 Comenzar con {categoryConfig.label}</span>
                      </span>
                    </button>
                  </div>
                </div>
        ...
        {!loading && !error && courses.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-white mb-6 flex items-center space-x-2">
              <span>🎯</span>
              <span>Ruta de Aprendizaje {categoryConfig.label}</span>
            </h2>
              ...
              <div className="space-y-4">
                {courses.map((course, index) => (
                  <div
                    ...
                    className="flex items-center space-x-4 cursor-pointer hover:bg-slate-700/30 p-3 rounded-lg transition-colors"
                  >
                    <div className={`w-8 h-8 bg-gradient-to-r ${categoryConfig.color} rounded-full flex items-center justify-center text-white font-bold text-sm`}>
                      {index + 1}
                    </div>
                    <div className="flex-1">
                      <h4 className="text-white font-semibold">{course.course_name}</h4>
                      <p className="text-gray-400 text-sm">{course.summary_30s || course.description}</p>
                    </div>
                    <span className="text-gray-500 text-sm">{course.estimated_time || 'N/A'}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {!loading && !error && courses.length === 0 && (
          <div className="mb-12 bg-slate-800/40 border border-slate-700/50 rounded-xl p-8 text-center">
            <p className="text-gray-400 text-lg">No hay cursos disponibles en esta categoría.</p>
          </div>
        )}
              ...
              {featuredCourses.map((course) => (
                ...
                >
                  ...
                  <div className="flex items-start space-x-6">
                    <div className={`w-20 h-20 bg-gradient-to-r ${course.color || categoryConfig.color} rounded-2xl flex items-center justify-center text-3xl group-hover:scale-110 transition-transform duration-300`}>
                      {course.icon || categoryConfig.emoji}
                    </div>
              ...
              {filteredCourses.map((course) => (
                ...
                >
                  <div className="flex items-center space-x-4 mb-4">
                    <div className={`w-16 h-16 bg-gradient-to-r ${course.color || categoryConfig.color} rounded-xl flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300`}>
                      {course.icon || categoryConfig.emoji}
                    </div>
                    <div className="flex-1 min-w-0">
...
const CategoryCourses: NextPage = () => {
  ...
  return <CategoryCoursesPage user={user} signOut={signOut} />
}
